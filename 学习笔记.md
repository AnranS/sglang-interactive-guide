...
浏览器实验请打开同目录的「打开学习指南.html」。模拟数据不是性能实测。

## 00 从一个请求开始

沿着 Prompt → Token 的路径，建立推理服务的第一张地图。
...
答案：2。调度器根据队列、token 预算和内存约束组织执行批次。

动手：单步走完流程，用自己的话说明首个 token 返回前做了哪些工作。

源码：https://github.com/sgl-project/sglang/blob/5200508b0fd25733752c2c5e3af5539508023c27/python/sglang/srt/entrypoints/http_server.py

官方文档：https://docs.sglang.io/docs/get-started/quickstart

## 01 架构与源码地图

将接口、调度、执行和缓存连接到真实源码入口。
...
答案：2。追踪数据流能建立模块之间的因果关系，再逐层展开优化路径。

动手：打开源码入口，找到 TokenizerManager、Scheduler 和 ModelRunner，记录各自的输入与输出。

源码：https://github.com/sgl-project/sglang/blob/5200508b0fd25733752c2c5e3af5539508023c27/python/sglang/srt/managers/scheduler.py

官方文档：https://docs.sglang.io/docs/basic_usage/overview

## 02 Prefill 与 Decode

同一个模型的两种工作负载，决定了延迟与吞吐的权衡。
...
答案：1。Prefill 要处理尚未缓存的输入 token，真实 TTFT 还受排队和硬件影响。

动手：把输入从 8 调到 32，保持输出不变，解释为什么不能按 token 数直接推算毫秒。

源码：https://github.com/sgl-project/sglang/blob/5200508b0fd25733752c2c5e3af5539508023c27/python/sglang/srt/model_executor/model_runner.py

官方文档：https://docs.sglang.io/docs/advanced_features/hyperparameter_tuning

## 03 连续批处理与调度

让完成的请求及时离场，让等待的请求补上空位。
...
答案：2。允许已完成请求离场并接纳等待请求，减少固定分组的空闲。

动手：分别运行两种模式直到结束，比较空槽；再说明真实服务中还缺少哪两个约束。
...
计算 GQA 的 KV 占用应使用哪个 head 数？

1. query heads
2. KV heads
3. 请求数

答案：2。K/V 的形状由 KV heads 决定。实验公式只适用于标注的普通 dense 情形。

动手：记录 4K 和 16K 上下文的估算值，再解释为何这不等于 GPU 总显存需求。

源码：https://github.com/sgl-project/sglang/blob/5200508b0fd25733752c2c5e3af5539508023c27/python/sglang/srt/mem_cache/memory_pool.py

官方文档：https://docs.sglang.io/docs/advanced_features/quantized_kv_cache

## 05 RadixAttention 与前缀复用

相同的 token 前缀，为什么可以少算一次。
...
两个请求的开头 token 不同，后面的相同句子能直接当作共享前缀吗？

1. 可以，含义接近即可
2. 不可以，前缀需从起点连续匹配
3. 只要 temperature 相同就可以

答案：2。因果注意力的 KV 依赖之前的上下文，后缀相同并不保证对应 KV 相同。

动手：依次插入两条相同系统前缀、不同末尾的请求，再修改第一个 token，观察命中数。

源码：https://github.com/sgl-project/sglang/blob/5200508b0fd25733752c2c5e3af5539508023c27/python/sglang/srt/mem_cache/radix_cache.py

官方文档：https://docs.sglang.io/docs/advanced_features/radix_eviction_policy

## 06 Chunked Prefill 与重叠调度

把长输入分段，把 CPU 与 GPU 的工作衔接起来。
...
减小 chunk 大小必然让所有延迟都变小吗？

1. 必然
2. 不一定，调度开销与排队会变化
3. 只影响模型准确率

答案：2。更细的调度粒度带来更多机会，也可能增加开销，必须结合负载测量。

动手：比较块大小 4、8、10 的分块结果，找出最后一块不足预算的情况。
...
理解 logits 如何成为 token，以及格式约束在哪生效。
...
JSON Schema 约束能够保证什么？

1. 输出事实永远正确
2. 支持范围内的结构约束
3. 请求无需模型执行

答案：2。结构合规和内容正确是两件事；业务层仍需检查语义。

动手：降低温度，再屏蔽最高分候选，解释概率为什么重新归一化。

源码：https://github.com/sgl-project/sglang/blob/5200508b0fd25733752c2c5e3af5539508023c27/python/sglang/srt/layers/sampler.py

官方文档：https://docs.sglang.io/docs/advanced_features/structured_outputs
...
1. 模型参数量
2. 重复的 CPU kernel 启动开销
3. 所有跨卡通信

答案：2。重放图降低启动成本，不能替代模型计算或自动消除通信。

动手：切换 1/2/4/8 个 rank，解释为什么分配更少 head 不代表延迟严格等比例下降。

源码：https://github.com/sgl-project/sglang/blob/5200508b0fd25733752c2c5e3af5539508023c27/python/sglang/srt/model_executor/runner/decode_cuda_graph_runner.py

官方文档：https://docs.sglang.io/docs/advanced_features/attention_backend
...
答案：2。要测量总成本以及实际输出 token 数，再与相同负载基线比较。

动手：将接受数从 3 调到 0，解释为何丢弃的候选也已经消耗了计算资源。

源码：https://github.com/sgl-project/sglang/blob/5200508b0fd25733752c2c5e3af5539508023c27/python/sglang/srt/speculative

官方文档：https://docs.sglang.io/docs/advanced_features/speculative_decoding

## 10 PD 分离与分层缓存

当服务扩展到多设备时，数据搬运成为一等问题。
...
答案：1。Decode 需要与请求一致的 KV，必须考虑传输链路及其开销。

动手：比较同样 1 GiB KV 在不同有效带宽下的理想传输时间，列出公式没包含的成本。

源码：https://github.com/sgl-project/sglang/blob/5200508b0fd25733752c2c5e3af5539508023c27/python/sglang/srt/disaggregation

官方文档：https://docs.sglang.io/docs/advanced_features/pd_disaggregation

## 11 启动服务与 API 实战

在合适的运行环境，把概念连到一次真实请求。
...
答案：2。连接拒绝是连接层问题，首先确认服务已成功监听目标端口。

动手：在符合要求的运行环境执行最小请求，记录模型、SGLang 版本、硬件和返回结果。
...
两组 benchmark 哪种比较才可信？

1. 只比较最好看的吞吐数字
2. 固定模型、硬件与负载，并记录缓存状态
3. 一组用短输入，一组用长输入

答案：2。控制变量和保留环境信息，才能把指标变化关联到机制。

动手：完成一张实验记录：环境、唯一变量、假设、指标、结果、解释、限制。

```
# 查看你安装版本支持的参数，再选择负载配置
...
源码：https://github.com/sgl-project/sglang/blob/5200508b0fd25733752c2c5e3af5539508023c27/python/sglang/bench_serving.py

官方文档：https://docs.sglang.io/docs/developer_guide/bench_serving
